...
const Checkout = () => {
  ...
  const handlePayNow = async () => {
    if (Object.values(address).some((v) => !v.trim() && v !== address.address_line2)) {
    ...
    const data = (await api.post("http://localhost:5000/api/payment/razorpay", {
      amount: totalAmount,
      currency: "INR",
    ...
    const options = {
      ...
      theme: { color: "#fb923c" }, // orange-pink gradient color
    };
  ...
  if (productLoading || orderLoading)
    return <p className="text-center mt-20 text-lg animate-pulse">Loading...</p>;
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-6 flex flex-col md:flex-row gap-6 border border-indigo-100">
            ...
            <div className="flex-1 space-y-4">
              ...
              <div className="flex items-center gap-4 text-gray-500 text-sm">
                <span>⭐ {product?.average_rating} ({product?.rating_count} reviews)</span>
              ...
              <div className="mt-2 flex items-center gap-2">
                ...
                <input
                  ...
                  max={product?.stock || 1}
                  onChange={(e) =>
                    setQuantity(Math.min(product?.stock || 1, Math.max(1, Number(e.target.value))))
                  }
        ...
        {showAddressForm && (
          <div className="fixed inset-0 flex justify-center items-center bg-black/50 z-50">
            <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-lg transform transition-all scale-95 animate-fade-in">
              <h2 className="text-2xl font-bold mb-4 text-indigo-600">Shipping Address</h2>
                ...
                {[
                  ...
                  <input
                    ...
                    className={`border border-indigo-200 rounded-lg px-3 py-2 col-span-2 ${
                      ["city", "state", "postal_code", "country"].includes(name) ? "col-span-1" : ""
